...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ExtCtrls, uGlobalData, uConstData,
  Vcl.StdCtrls, Vcl.ComCtrls, udictionary;
...
type
  TForm5 = class(TForm)
    ...
    procedure CreateListViewGarden(gardenlist: PtGarden);
...
procedure TForm5.CreateListViewGarden(gardenlist: PtGarden);
var
  listitem: tlistitem;
  strColor: string;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if FrequensChars.ContainsKey(gardenmas[i][j].—ulture.cod) then
          begin
            FrequensChars[gardenmas[i][j].—ulture.cod] :=
              FrequensChars[gardenmas[i][j].—ulture.cod] + 1;
          end
          else
          begin
            FrequensChars.Add(gardenmas[i][j].—ulture.cod, 1);
          end;

          // showmessage(inttostr(idGarden[k]) + ' : ' + inttostr(FrequensChars[gardenmas[i][j].—ulture.cod]));
        end;
      end;
    end
  end;
...
begin
  ...
  PaintBox1.Width := (1 + _GardenX) * (SizeRect);
  PaintBox1.Height := (1 + _GardenY) * (SizeRect);
